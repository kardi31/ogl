...
if($_POST['tresc']!="")
{
  $in="INSERT INTO ".$pre."komentarze(`kom_nick`, `kom_tresc`, `kom_data`, `kom_typ`, `kom_idk`, `kom_idu`) VALUES ('".htmlspecialchars($nick)."', '".htmlspecialchars($_POST['tresc'])."', NOW(), 'z', '".htmlspecialchars($_GET['idf'])."', '".$_SESSION['user_id']."')";
  db_query($in);
}
else
{
...
$kom_ile=0;
$Query='SELECT * FROM '.$pre.'komentarze WHERE kom_idk="'.db_real_escape_string($_GET['idf']).'" AND kom_typ="z" ORDER by kom_id DESC'; 
...
$smarty->assign("oceniono",$_COOKIE['fo_'.$news_id.'']);
...
$smarty->assign("title",$lang['469'].' '.$userlo.' - '.$ust['nazwa']);

$smarty->display($ust['templates'].'/profil-zdjecia.tpl');
